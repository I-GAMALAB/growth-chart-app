...
GC.get_data = function () {
  ...
  function isValidObservation(obj) {
    if (
      !obj.hasOwnProperty("valueQuantity") ||
      !obj.valueQuantity ||
      typeof obj.valueQuantity != "object"
    ) {
      return false;
    }

    if (
      !obj.valueQuantity.hasOwnProperty("value") ||
  ...
  function patientAgeInMonths(d, patient) {
    return (
      -1 * new XDate(d).diffMonths(new XDate(patient.demographics.birthday))
    );
  }
  ...
  function fetchFamilyHistory(client) {
    return client
      .request("FamilyMemberHistory?patient=" + client.patient.id, {
        pageLimit: 0,
        flat: true,
      })
      .catch(function () {
  ...
  function processBoneAge(client, boneAgeValues, arr) {
    boneAgeValues &&
      boneAgeValues.forEach(function (v) {
  ...
  function processVitals(client, patient, vitalsByCode) {
    function process(observationValues, toUnit, arr) {
      observationValues &&
        observationValues.forEach(function (v) {
          if (isValidObservation(v)) {
            arr.push({
    ...
    process(vitalsByCode["9843-4"], units.cm, vitals.headCData);
  ...
  function onReady(client) {
    ...
    var hidePatientHeader =
      client.getPath(client, "state.tokenResponse.need_patient_banner") ===
      false;
